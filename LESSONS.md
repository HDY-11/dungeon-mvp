...
**问题背景：** `line_bresenham(x0,y0, x1,y1)` 在 `x0==x1 && y0==y1` 时，方向 `sx = if x0 < x1 { 1 } else { -1 }` 因 `x0<x1` 为假而得到 `-1`，每一步朝远离目标的方向走，永不终止。坐标递减至负值后 `as usize` 回绕导致越界 panic。

**错误做法：** 假设起点一定不等于终点，或依赖调用方避免传入相同坐标。
...
**为什么更好：** 方向派生自比较（而非显式 `dx.signum()`）的算法中，start==end 时所有比较均为假，推导出"反向"步进。这不是数学错误——Bresenham 算法在零长度输入时的行为是未定义的，但实现者通常只测试了非零路径。一个函数入口的早期返回即可消除整个类别的问题，比要求所有调用方记得不传相同坐标更可靠。
