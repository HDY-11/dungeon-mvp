...
- 不引入 `CanThrow` 组件——玩家投掷由按键触发，无"决策系统检查是否该投掷"的场景
...
**关联 Issue：** ISSUES.md #A12 #D14 #D14A #A18 #I31 ⑦ #G21
...
### Dsn19 模板/碎片/核心合成系统 — 设计方向（讨论阶段）

**决策**

解决 G17（材料无消耗渠道）的核心方案。采用 **模板 + 碎片 + 核心** 三元素合成系统。
...
**关键设计点：**
- 碎片不是"残缺品"而是"窄谱模板"——独立可用，不依赖收集
- 碎片和完整模板都占背包空间（制造背包抉择）
- 碎片品质高于核心承载上限时，不能被核心吸纳（保留独立使用）
- 全能模板不能合"所有"东西——给传说级留专有地盘
- 分类（生存/合成/强化/魔法）仅存在于基础层，高级阶段归一化
- 解锁更高级模板的关键是获取**模板核心**，碎片补全完整度
- 某些碎片永远无法被整合进任何完整模板（世界深度设计）

**当前状态：** 讨论阶段，尚未开始实现。代码层准备：`ItemMeta.tier` 字段已存在（I41），`ItemBehavior` trait 骨架已定义（I40）但未被消费。需先实现首个交互闭环。
