...
### I27 — unique_color 色差算法无效：相邻实体 ID 颜色无区别 ✅已修复

**修复前：** `unique_color` 取 `entity.to_bits()` 的低 6 位作为偏移量（±32），相邻实体 ID（如 0 和 1）的低位几乎相同，产生肉眼不可见的色差。

**修复后：** 使用黄金比例常数 `0x9E3779B97F4A7C15` 做 `wrapping_mul` 将相邻 ID 扩散到全 64 位空间，每通道偏移范围扩大到 ±64，同种怪物间颜色差异明显。

**位置：** `dungeon-render/src/color.rs`
...
### G14 — 玩家与楼梯重合（G9 回归，修复两次） ✅已修复

**第一次修复（I19，与 G9/G10/I16 同期）：** 提取 `pick_stair_pos` 共享函数，尾部加入醉汉游走回退代码，用于单房间时在 ≥15 格外找楼梯位置。

**问题：** 醉汉游走在 `farthest_room_from` 之后，而 `farthest_room_from` 对任意非空 rooms 都返回 `Some`。单房间时返回 `rooms[0].center()`（出生点本身），函数直接 return——**醉汉游走永远不执行，是死代码。** 测试未覆盖单房间场景，未发现回退路径不可达。

**第二次修复（G14）：** `pick_stair_pos` 增加 `map.rooms.len() > 1` 守卫。仅多房间时走 farthest_room_from；单房间时直接进入醉汉游走，确保楼梯距出生点 ≥15 格。

**教训：** 当有"正常路径"和"退化回退"两条逻辑路径时，应确认退化路径确实可达。死代码比没有代码更有害——它让你以为已经处理了边界情况。
