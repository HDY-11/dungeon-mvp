...
/// 对称阴影投射视野计算
pub fn calculate_visible_tiles(x: usize, y: usize, range: usize, map: &crate::Map) -> Vec<(usize, usize)> {
    ...
    let mut is_blocking = |pos: (isize, isize)| {
        if pos.0 < 0 || pos.0 >= crate::MAP_WIDTH as isize || pos.1 < 0 || pos.1 >= crate::MAP_HEIGHT as isize {
            return true;
        }
